...
test('waitForStartupCatchUp resolves only after catch-up doctor work finishes', async () => {
  const { db, tmpDir } = createTempDb()
  const now = new Date().toISOString()

  db.insert(projects).values({
    id: 'proj_doctor_catchup',
    name: 'doctor-catchup',
    displayName: 'Doctor Catchup',
    canonicalDomain: 'example.com',
    country: 'US',
    language: 'en',
    createdAt: now,
  ...
  db.insert(schedules).values({
    id: 'sched_doctor_catchup',
    projectId: 'proj_doctor_catchup',
    kind: 'doctor',
    cronExpr: '0 */6 * * *',
    timezone: 'UTC',
    enabled: true,
  ...
  const doctorWork = new Promise<void>((resolve) => {
    releaseDoctor = resolve
  })
  const scheduler = new Scheduler(db, {
    onRunCreated: () => {},
    onDoctorRequested: () => doctorWork,
  })

  scheduler.start()
  let catchUpDone = false
  const waiting = scheduler.waitForStartupCatchUp().then(() => {
    catchUpDone = true
  })
  await Promise.resolve()
  expect(catchUpDone).toBe(false)
