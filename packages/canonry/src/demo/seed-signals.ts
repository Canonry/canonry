...
function seedProjectSignals(db: DatabaseClient, project: DemoSeedProject, now: Date, variant: number): void {
  ...
  db.insert(gscCoverageSnapshots).values({ id: `${prefix}-gsc-coverage`, projectId: project.id, syncRunId, date: dates.at(-1)!, indexed: 51, notIndexed: 3, unknownPages: 2, verifiedByInspection: 4, derivedFromImpressions: 47, reasonBreakdown: { 'Sample: excluded by noindex': 2, 'Sample: redirect': 1 }, createdAt: nowIso }).run()
  ...
  db.insert(insights).values([
    { id: `${prefix}-insight-1`, projectId: project.id, runId: syncRunId, type: 'content-gap', severity: 'medium', title: 'Expand service comparison guidance', query: searchTerms[0]!, provider: 'openai', recommendation: { action: 'Draft a comparison section', target: `${root}services/`, reason: 'Sample insight for the public dashboard.' }, cause: { cause: 'Sample answer-history gap', details: 'This is stored sample data.' }, dismissed: false, createdAt: nowIso },
    { id: `${prefix}-insight-2`, projectId: project.id, runId: syncRunId, type: 'technical', severity: 'low', title: 'Review one internal link target', query: searchTerms[1]!, provider: 'perplexity', recommendation: { action: 'Repair the illustrative link', target: `${root}guides/`, reason: 'Sample finding for the public dashboard.' }, cause: { cause: 'Sample crawl finding', details: 'This is stored sample data.' }, dismissed: false, createdAt: nowIso },
  ]).run()
  db.insert(auditLog).values([
    { id: `${prefix}-audit-1`, projectId: project.id, actor: 'demo-fixture', action: 'demo.seeded', entityType: 'project', entityId: project.id, diff: 'Synthetic public demo signals seeded; no provider was contacted.', createdAt: nowIso },
    { id: `${prefix}-audit-2`, projectId: project.id, actor: 'demo-fixture', action: 'demo.site-health.snapshot', entityType: 'site-crawl', entityId: crawlRunId, diff: 'Synthetic completed crawl graph and findings stored.', createdAt: nowIso },
  ]).run()
}
...
function seedLocalAndCommercialSignals(db: DatabaseClient, input: { project: DemoSeedProject; prefix: string; root: string; syncRunId: string; campaignId: string; groupId: string; locationName: string; nowIso: string; dates: string[] }): void {
  ...
  db.insert(backlinkSummaries).values({ id: `${prefix}-backlinks`, projectId: project.id, source: 'commoncrawl', release: 'DEMO-2026-09', targetDomain: project.domain, totalLinkingDomains: 27, totalHosts: 38, top10HostsShare: '0.54', queriedAt: nowIso, createdAt: nowIso }).run()
  ...
  db.insert(conversionTrackingContracts).values({ id: `${prefix}-conversion`, projectId: project.id, name: 'Sample lead', eventName: 'generate_lead', googleAds: { customerId: 'demo-customer', conversionActionId: 'demo-action', conversionId: 'demo-conversion-id', conversionLabel: 'demo-label', campaignIds: [campaignId], requireBiddableGoal: true, requirePrimaryAction: true }, gtm: { accountId: 'demo-account', containerId: 'demo-container', tagId: 'demo-tag', triggerIds: ['demo-trigger'], variableIds: ['demo-value', 'demo-currency'] }, runtime: { verificationRequired: false, requireTransactionId: false, requireValue: true, requireCurrency: true, productionHosts: [project.domain] }, createdAt: nowIso, updatedAt: nowIso }).run()
}
