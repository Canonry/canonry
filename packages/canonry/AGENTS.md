...
| `src/mcp/tool-registry.ts` | All API tools (135, including `canonry_analytics_sources`, `canonry_visibility_stats`, `canonry_visibility_compare`, `canonry_content_brief`, `canonry_content_map`, `canonry_report`, `canonry_analytics_metrics`, `canonry_gbp_places`, `canonry_backlinks_sources`, the guarded ads lifecycle and activation-recovery tools, and the `canonry_technical_aeo_*` site-audit tools), each tagged with a `tier` (`core` or one of the toolkit names) |
