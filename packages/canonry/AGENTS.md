...
| `src/commands/discover.ts` | Discovery commands: `discover run` (kick off ICP → seed → embed → probe pipeline), `discover seed` (alias for run today; the phase split is a later PR), `discover list` (sessions newest-first), `discover show` / `discover probe` (session detail with per-query probes), `discover promote preview` (read-only preview of bucketed queries + suggested new competitor domains), `discover promote` (adopt a completed session's queries + competitors into the project — add-only, idempotent, with `--bucket` / `--no-competitors` scoping). |
