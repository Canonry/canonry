...
| `src/mcp/tool-registry.ts` | All API tools (100, including `canonry_content_brief`, `canonry_content_map`, `canonry_report`, `canonry_analytics_metrics`, and `canonry_gbp_places`), each tagged with a `tier` (`core` or one of the toolkit names) |
