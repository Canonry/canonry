...
| `src/commands/discover.ts` | Discovery commands: `discover run` (kick off ICP → seed → embed → probe pipeline), `discover seed` (alias for run today; will split in PR 2), `discover list` (sessions newest-first), `discover show` / `discover probe` (session detail with per-query probes), `discover promote preview` (read-only preview of bucketed queries + suggested new competitor domains; PR 2 ships the actual `promote` write). |
| `src/cli-commands/discover.ts` | CLI specs for `discover run / seed / probe / list / show / promote preview`. |
