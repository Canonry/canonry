...
| `src/commands/gbp.ts` | Google Business Profile commands: `gbp connect` / `disconnect`, `gbp locations` (list / discover / select / deselect), and Phase 2 performance: `gbp sync` (fires the `gbp-sync` run; `--wait` polls to terminal), `gbp metrics` (stored daily metrics, totals-by-metric), `gbp keywords` (stored monthly impressions with `<N` threshold rendering + thresholded %). Reviews are NOT here — the v4 reviews API is separately access-gated by Google; Q&A was retired. |
| `src/gbp-sync.ts` | `executeGbpSync` — pulls daily metrics (30d) + monthly keyword impressions (12mo) for a project's selected locations, bounded-concurrency 4 per location, range-replaces each location's rows in one transaction, captures per-location errors → run status completed/partial/failed. |
