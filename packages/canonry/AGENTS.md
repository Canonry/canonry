...
| `src/mcp/tool-registry.ts` | All API tools (110, including `canonry_analytics_sources`, `canonry_content_brief`, `canonry_content_map`, `canonry_report`, `canonry_analytics_metrics`, `canonry_gbp_places`, and the `canonry_technical_aeo_*` site-audit tools), each tagged with a `tier` (`core` or one of the toolkit names) |
...
| `src/commands/ads.ts` | OpenAI ads commands: `ads connect --api-key` / `disconnect` / `status` / `sync` / `campaigns` (jsonl) / `insights` (jsonl; `--level/--entity/--from/--to`) / `summary`. Money renders via `formatMicros`. Registered via `src/cli-commands/ads.ts`. |
