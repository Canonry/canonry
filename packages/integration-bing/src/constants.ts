...
// `GetLinkCounts` / `GetUrlLinks` are paginated (each response carries
// `TotalPages`). One page == one API call, so cap how many pages a single
// auto-paginating call will walk to keep the Bing daily request budget bounded.
export const BING_LINKS_MAX_PAGES = 20
