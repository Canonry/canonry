...
test('v160 adds nullable server-side return targets without disturbing existing Google login transactions', () => {
  const db = createClient(':memory:')
  databases.push(db)
  migrate(db, MIGRATION_VERSIONS.filter(migration => migration.version <= 159))
