...
function renderOpportunities(report: ProjectReportDto): string {
  ...
  const highlights = `<div class="opportunity-grid">
    ${opps.slice(0, 3).map(o => `<article class="opportunity-card">
      <div class="opportunity-score">${Math.round(o.score)}</div>
      <h3>${escapeHtml(o.query)}</h3>
      <p>${escapeHtml(o.action)} · ${escapeHtml(o.actionConfidence)} confidence</p>
  ...
  return section(
    {
      ...
      intro: 'Queries where content work has the clearest path to more AI citations.',
    },
    ...
    ${actions.map(action => {
      ...
      return `<article class="action-card">
        <div class="action-head">
          <div class="action-rank">${formatNumber(action.priority)}</div>
          <div>
