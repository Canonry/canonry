...
function renderCitationScorecard(report: ProjectReportDto): string {
  ...
  `
  return section(
    { id: 'citation-scorecard', eyebrow: 'Section 2', title: 'Citation Scorecard', intro: 'Per-query × per-provider citation matrix from the latest visibility sweep.' },
    body,
  )
}
...
function renderCompetitorLandscape(report: ProjectReportDto): string {
  ...
  const table = competitors.length > 0
    ? `<table class="report-table">
        <thead><tr><th>Domain</th><th>Pressure</th><th>Citations</th><th class="numeric">SOV</th><th>Cited queries</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>`
