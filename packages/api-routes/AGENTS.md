...
| `src/session.ts` | Cookie-backed browser session plugin shared by `canonry serve` and `apps/api`: `GET/POST/DELETE /session`, `POST /session/setup`. Salted-scrypt dashboard passwords (legacy SHA-256 transparently rehashed on login), pluggable `DashboardPasswordStore` (config.yaml locally, `app_settings` row on Cloud Run), in-memory `SessionStore`, scoped `@fastify/rate-limit` brute-force guard (login/setup 10/min). `setupRequiresApiKey` gates first-run setup behind a valid bearer on network-reachable binds (#690 port) — `canonry serve` derives it from `isLoopbackBindHost(host)`, `apps/api` sets it unconditionally |
| `src/guest-report.ts` | Anonymous guest-report funnel (`/guest/report*`): create + poll + SSE stream are anonymous, claim requires auth. Entirely OFF unless `CANONRY_ENABLE_GUEST_REPORTS` is set (404s, fingerprint-resistant). Default driver is the bundled demo simulator — every response carries `simulated: boolean` so consumers can label demo output; a real driver injects via `GuestReportRoutesOptions.driver`. Scoped rate limits (create 15/min, claim 20/min, reads 60/min). Claim is first-writer-wins (`claimed_at IS NULL` guard); create/claim audit-logged |
| `src/cloud/` | Track 3 cloud bridge (`/cloud/bootstrap`, `/cloud/google/import-tokens`, `/cloud/bing/import-key`): double-gated (`CANONRY_ENABLE_CLOUD_BOOTSTRAP` env → 404 when unset; `X-Admin-Scope: 1` header → intent gate, not authz). Bootstrap upserts the `cloud_metadata` singleton + a tenant-scoped (`projectId NULL`) notification subscriber tagged `source: 'cloud-bootstrap'` — re-bootstrapping with a new callback URL retires stale bootstrap subscribers. `emit-connection-event.ts` dispatches signed `CloudWebhookPayload` events outbound (inert with zero subscribers — every OSS deployment). Deliberately excluded from OpenAPI via the explicit allowlist in `openapi-contract.test.ts` |
