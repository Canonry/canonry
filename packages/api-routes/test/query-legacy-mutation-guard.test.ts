...
describe('legacy query catalog mutation guard', () => {
  ...
  it('rejects additions-only replacements while a planless run is queued', async () => {
    const { app, db } = buildApp()
    const projectId = seedProject(db, 'queued-addition')
    seedQuery(db, projectId, 'Existing question')
    db.insert(runs).values({
      id: crypto.randomUUID(), projectId, kind: 'answer-visibility',
      status: RunStatuses.queued, trigger: 'manual', createdAt: NOW,
    }).run()
    await app.ready()
    const response = await app.inject({
      method: 'PUT', url: '/api/v1/projects/queued-addition/queries',
      payload: { queries: ['Existing question', 'New question'] },
    })
    expect(response.statusCode).toBe(409)
    expect(db.select().from(queries).where(eq(queries.projectId, projectId)).all())
      .toMatchObject([{ query: 'Existing question' }])
  })

  it('fills missing historical text before duplicate collapse and casing-only relabels', async () => {
    const { app, db } = buildApp()
    const projectId = seedProject(db, 'historical-duplicates')
    const keptId = seedQuery(db, projectId, 'Original QUESTION')
    const duplicateId = seedQuery(db, projectId, 'Original question')
    const keptSnapshot = seedSnapshot(db, projectId, keptId, null)
    const duplicateSnapshot = seedSnapshot(db, projectId, duplicateId, null)
    await app.ready()
    const response = await app.inject({
      method: 'PUT', url: '/api/v1/projects/historical-duplicates/queries',
      payload: { queries: ['original question'] },
    })
    expect(response.statusCode).toBe(200)
    expect(db.select().from(querySnapshots).where(eq(querySnapshots.id, keptSnapshot)).get())
      .toMatchObject({ queryText: 'Original QUESTION' })
    expect(db.select().from(querySnapshots).where(eq(querySnapshots.id, duplicateSnapshot)).get())
      .toMatchObject({ queryText: 'Original question' })
  })
})
