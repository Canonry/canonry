...
For a configured portfolio slice, select one `--market <key>`, `--group <key>`, or `--property <key>`; Canonry saves that context with the answers. Research never adds queries to the tracked questions.
