...
// (`Math.round(x) + '%'`) or JSX text (`{x.toFixed(0)}%`). A `%` after an
// unrounded value is left alone, because that is how CSS geometry is written
// (`width: \`${share * 100}%\``) and display code always rounds.
    ...
    return {
      TemplateLiteral(node) {
        node.expressions.forEach((expression, index) => {
          if ((node.quasis[index + 1]?.value.cooked ?? '').startsWith('%') && roundsForDisplay(expression)) report(expression)
        })
      },
