...
**Keyword normalization:** When new keywords are added to a project mid-history, canonry automatically normalizes each time bucket to only include keywords that existed before that bucket started. This prevents newly-added (typically uncited) keywords from creating a false drop in the citation rate trend. The chart displays dashed vertical annotation lines at points where keywords were added (e.g. "+3 keywords"), and each bucket's tooltip shows the keyword count used for that bucket's calculation.
