...
6. Copy both renderers show lives in `packages/contracts/src/report-sections.ts` (`REPORT_SECTION_COPY` plus copy functions such as `reportExecutiveHeadline` and `reportServerActivityHeading`), alongside `report-visibility.ts` and `share-of-voice.ts`. Never write report copy inline in either renderer.
7. Section order comes from `reportSectionOrder(report, audience)`; the SPA renders it through an exhaustive switch over `ReportSectionIds`, and `report-renderer-bytes.test.ts` checks the two agree.
8. `packages/api-routes/test/report-renderer-bytes.test.ts` pins the HTML bytes and writes the outline goldens in `packages/api-routes/test/fixtures/report-outline/`; `apps/web/test/report-page.test.tsx` holds the SPA to them through its `data-report-*` hooks. Only the api-routes suite regenerates them, and never to make a failing test pass.
