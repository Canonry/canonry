...
# Schedules — one row per (project, kind) where kind ∈ {answer-visibility, traffic-sync, gbp-sync, data-refresh, backlinks-sync, site-audit, ads-sync, doctor, site-liveness}
...
canonry schedule show <project> [--kind answer-visibility|traffic-sync|gbp-sync|data-refresh|backlinks-sync|site-audit|doctor|site-liveness] # default kind is answer-visibility
...
| integrations | `ga.data.recent-data` | project | Newest stored GA4 daily row is no older than 3d (warns `ga.data.aging`) or 5d (fails `ga.data.stale`). Catches a sync that keeps succeeding with zero rows, for example a GA4 tag removed from the site; skipped when GA4 is not connected |
| integrations | `gsc.data.recent-data` | project | Newest stored Search Console daily row is no older than 5d (warns `gsc.data.aging`) or 7d (fails `gsc.data.stale`), above GSC's normal 2-3 day lag; skipped when GSC is not connected |
| integrations | `site.reachability` | project | The project homepage (`https://<canonicalDomain>/`) answers below HTTP 500, retried once. Every hop is resolved and checked against private, reserved and link-local ranges before dialing; a refused address is skipped, never probed. A 403 or 429 bot challenge counts as up |
...
- `doctor` (every 6h, seeded per project) grades every check and notifies when the worst `(status, code)` changes. It leaves `site.reachability` out of that grade.
- `site-liveness` (every 10 min, seeded per project) runs only `site.reachability`, keeps its own `site_liveness_state` row, pages after two failed passes in a row, and sends `health.recovered` only for an outage it paged. It never writes `doctor_health_state`, so a quick "site is up" pass cannot clear a GA outage.
