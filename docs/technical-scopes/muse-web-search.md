...
- Count citations only from `url_citation` annotations on final `output_text`. Keep retrieved `web_search_call.results` separate.
- Store the key under `providers.muse`; accept `MUSE_API_KEY` and Meta's `MODEL_API_KEY` alias. Expose `MUSE_MODEL`, `MUSE_BASE_URL`, and `init --muse-key`.
