...
| **ads_campaigns** | Campaign snapshots, range-replaced per project on every `ads-sync`. Includes description, flight dates, upstream lifecycle timestamps, budgets in integer micros, and raw upstream targeting (the API projects location IDs). Ids are upstream ids (`cmpn_…`). |
