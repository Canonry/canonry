...
| **ads_operations** | Durable receipts for upstream mutations. Unique `(project_id, operation_key)` plus a canonical request hash prevents duplicate sends and detects key reuse with a different request. State is `pending`, `succeeded`, `failed`, or `unknown`; ambiguous outcomes are never automatically retried. Stores sanitized error metadata, entity id, and upstream update timestamp, never the SDK key or request payload. |
| **ads_campaigns** | Campaign snapshots, range-replaced per project on every `ads-sync`. Includes description, flight dates, upstream lifecycle timestamps, budgets in integer micros, attached `conversion_event_setting_ids`, and raw upstream targeting (the API projects location IDs). Ids are upstream ids (`cmpn_…`). |
| **ads_ad_groups** | Ad-group snapshots incl. description and upstream lifecycle timestamps. `context_hints` (JSON `string[]`) is the targeting primitive — entries are multi-line strings of newline-separated example queries; the paid/organic overlap matcher joins these against tracked queries. Cascades off `ads_campaigns`. |
