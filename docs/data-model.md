...
| **ads_ad_groups** | Ad-group snapshots. `context_hints` (JSON `string[]`) is the targeting primitive — one example query per entry (the live API returns them newline-joined in a single string; the sync splits on newlines at ingest); the paid/organic overlap matcher joins these against tracked queries. Cascades off `ads_campaigns`. |
