...
Power-user environments (scripts, Aero, telemetry harnesses) that want the flat 74-tool catalog at startup, including the two meta-tools, can opt back in with `--eager` (or `CANONRY_MCP_EAGER=1`):
