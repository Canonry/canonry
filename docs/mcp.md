...
| --- | --- | --- |
| `monitoring` | runs list/latest/get, project history, timeline, snapshots list/diff, insights list/get, health latest/history, content targets/sources/gaps, `canonry_report` (aggregated AEO report bundle), `canonry_portfolio` (cross-project change feed + recent runs + state) | Investigating regressions, comparing runs, reviewing insights/health, surfacing content opportunities, the "what changed across all my projects" daily check, generating client-facing reports |
