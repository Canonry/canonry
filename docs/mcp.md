...
`canonry_apply_config` accepts one config-as-code project document per call. For multi-document YAML or multiple project files, agents should call the tool once per project document. `canonry_queries_generate` returns suggestions only; persist accepted suggestions with `canonry_queries_add` or replace the tracked set with `canonry_queries_replace`. `canonry_query_edit` (or `canonry query edit`) replaces one Simple-tracking query with a new identity, guarded by its exact saved text; it does not alter a plan or start a run. The `canonry_keywords_*` tools remain as legacy aliases over the same query store for older clients.

Generic replacements, removals, and config apply reject changes to queries referenced by an active plan or draft. Use draft actions for those changes. Unchanged queries and unassigned catalog cleanup remain supported.
