...
`canonry_traffic_referral_assessment` is a read-only traffic tool backed by
`GET /projects/:name/traffic/referral-assessment` and
`canonry traffic referral-assessment`. Supply `project`, `startDate`, and
`endDate` (inclusive UTC dates, at most 366 days). Optional `sourceId`,
`burstThreshold` (default 100), `ratioThreshold` (default 3), and `limit`
