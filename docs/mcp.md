...
v1 is curated for client usability: 72 API tools (48 read in `--read-only`) plus two meta-tools (`canonry_help`, `canonry_load_toolkit`). It covers projects, project-overview and search composites, config apply, runs, snapshots, insights, health, query generation and replacement, legacy keyword aliases, competitor add/remove, schedules, settings, GSC reads, GA reads, server-side traffic ingestion (Cloud Run connect/sync + crawler/AI-referral rollup reads), the doctor health-check (Google/GA auth diagnostics), run trigger/cancel, schedule updates, insight dismiss, content gap/target/source analysis, backlinks domains, durable Aero memory (list/set/forget), agent transcript clear, and agent webhook attach/detach.
...
| `traffic` | List sources, source detail (24h totals + latest run), windowed crawler/AI-referral events, Cloud Run connect/sync | Confirming server-log evidence of crawler hits or AI-referral arrivals (e.g. GPTBot, ChatGPT-User), or wiring up / syncing a Cloud Run traffic source |
