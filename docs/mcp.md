...
| `discovery` | Start a discovery session (`canonry_discover_run_start`), list sessions, inspect a session's probes + buckets, preview a promotion plan, and promote a completed session's queries + competitors into the tracked basket (`canonry_discover_promote`) | Expanding or auditing a project's tracked-query basket, auditing competitive surface, adopting a discovered basket into the project |
