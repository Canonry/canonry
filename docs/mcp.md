...
v1 is curated for client usability: 78 API tools (52 read in `--read-only`) plus two meta-tools (`canonry_help`, `canonry_load_toolkit`). It covers projects, project-overview and search composites, config apply, runs, snapshots, insights, health, query generation and replacement, legacy keyword aliases, competitor add/remove, schedules, settings, GSC reads, GA reads, server-side traffic ingestion (Cloud Run connect/sync + async backfill + crawler/AI-referral rollup reads), the doctor health-check (Google/GA auth diagnostics), run trigger/cancel, schedule updates, insight dismiss, content gap/target/source analysis, backlinks domains, durable Aero memory (list/set/forget), agent transcript clear, agent webhook attach/detach, and the tracked-basket discovery pipeline (start a session, list sessions, inspect probes, preview a promotion plan).
...
The full 78-tool API catalog costs roughly 17k tokens of definitions every session. Most sessions touch a handful of tools, so `canonry-mcp` defaults to a small **core tier** (~10 tools, ~3k tokens) and registers the rest on demand via `notifications/tools/list_changed`.
...
| `traffic` | List sources, source detail (24h totals + latest run), windowed crawler/AI-referral events, Cloud Run connect/sync, async backfill (replaces hourly rollups in a `--days` window with current classifier output) | Confirming server-log evidence of crawler hits or AI-referral arrivals (e.g. GPTBot, ChatGPT-User), wiring up / syncing a Cloud Run traffic source, or one-shot reclassifying historical logs after a classifier change |
...
| `discovery` | Start a discovery session (`canonry_discover_run_start`), list sessions, inspect a session's probes + buckets, preview the promotion plan for PR 2 | Expanding or auditing a project's tracked-query basket, auditing competitive surface, previewing a promotion before merging |
