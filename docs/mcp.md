...
Connect to `/api/v1/mcp` on the instance for core tools plus monitoring, available immediately. Use `/api/v1/mcp/readonly` to force read-only tools even with a write-capable credential. A read-only credential also stays read-only on the default endpoint.

The default connection includes visibility reports, Property rankings, stored question results and competitors, changes, and data quality. Property overview and paged evidence are core reads, so every toolkit endpoint retains them. These measurement reads use stored results and never start a sweep.
...
- `canonry_measurement_overview`, `canonry_measurement_property_evidence` — scoped Property rankings and paged evidence
