...
`provider-gemini`, `provider-openai`, `provider-muse`, `provider-claude`, `provider-local`, `provider-perplexity`, `provider-cdp` — each implements `ProviderAdapter` from `contracts`.
