...
// ─── Grounding-source domain extraction (mirrors packages/provider-gemini/src/normalize.ts) ───
// Gemini grounding URLs come back as opaque vertexaisearch.cloud.google.com
// redirects. The provider extractor decodes the redirect path when it's a
// base64-encoded URL, otherwise falls back to extracting the destination
// domain from the `title` field (which Google reliably populates). Without
// this fallback, all Gemini grounding entries collapse into the single
// "vertexaisearch.cloud.google.com" host — completely wrong.
function extractGroundingDomain(g: unknown): string | null {
  if (!g || typeof g !== 'object') return null
  const obj = g as { uri?: unknown; title?: unknown }
  ...
  if (uri) {
    try {
      ...
      if (hostname === 'vertexaisearch.cloud.google.com') {
        // Vertex redirect — try to decode the path, otherwise fall through to title.
        ...
        if (redirectPath && redirectPath !== url.pathname) {
          try {
            ...
            if (decoded.startsWith('http')) {
              return normalizeDomain(new URL(decoded).hostname)
            }
  ...
  for (const project of projects) {
    ...
    for (const snap of latestSnapshots) {
      ...
      for (const g of grounding) {
        const d = extractGroundingDomain(g)
        if (d) groundingDomains.add(d)
      }
    ...
    for (const [key, stats] of byProvider) {
      ...
      for (const snap of matching) {
        ...
        for (const g of grounding) {
          const domain = extractGroundingDomain(g)
