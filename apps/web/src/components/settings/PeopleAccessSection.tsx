...
import { ApiError, createUserInvitation, fetchGoogleSignInSettings, fetchUserAccessHistory, fetchUserInvitations, fetchUsers, replaceUserInvitation, revokeUserAccess, revokeUserInvitation, updateUser, type ApiUser } from '../../api.js'
...
export const PEOPLE_ACCESS_COPY = {
  heading: 'People & access',
  setupNeeded: 'Set up Google sign-in before inviting people.',
  openSignIn: 'Open Sign-in settings',
  invitationHeading: 'Invitations',
} as const

export const PEOPLE_ACCESS_TEST_IDS = {
  section: 'people-access-section',
  peopleTable: 'people-access-table',
  invitationsTable: 'people-invitations-table',
  signInSetup: 'people-sign-in-setup',
} as const
...
/** Instance-level access administration. Google credentials deliberately live in Sign-in settings. */
export function PeopleAccessSection() {
  ...
  const signInReady = googleQuery.data?.enabled && googleQuery.data.configured

  return (
    <section className="page-section-divider" aria-labelledby="people-access-heading" data-testid={PEOPLE_ACCESS_TEST_IDS.section}>
      <div className="section-head"><div><p className="eyebrow eyebrow-soft">Instance access</p><h2 id="people-access-heading">{PEOPLE_ACCESS_COPY.heading}</h2></div></div>
      ...
      <form className="mt-4 flex flex-wrap items-end gap-2" onSubmit={asyncHandler(async event => { event.preventDefault(); await invite() })}>
        <label className="min-w-56 flex-1 space-y-1" htmlFor="invite-email"><span className="text-xs font-medium text-secondary">Email</span><input id="invite-email" className="w-full rounded-md border border-base bg-bg px-3 py-2 text-sm text-heading outline-none focus:border-mono-600" type="email" required value={email} onChange={event => setEmail(event.target.value)} /></label>
        <label className="space-y-1" htmlFor="invite-role"><span className="text-xs font-medium text-secondary">Role</span><select id="invite-role" className="rounded-md border border-base bg-bg px-3 py-2 text-sm text-heading" value={role} onChange={event => setRole(event.target.value as UserRole)}><option value={UserRoles.viewer}>Viewer</option><option value={UserRoles.analyst}>Analyst</option><option value={UserRoles.admin}>Admin</option></select></label>
        <Button type="submit" disabled={pending || !email.trim() || !signInReady}>Create invitation</Button>
      </form>
      {!signInReady && googleQuery.data ? <p className="mt-2 text-sm text-secondary" data-testid={PEOPLE_ACCESS_TEST_IDS.signInSetup}>{PEOPLE_ACCESS_COPY.setupNeeded} <a className="underline underline-offset-2 hover:text-heading" href="?section=sign-in">{PEOPLE_ACCESS_COPY.openSignIn}</a></p> : null}
      {googleQuery.isError ? <p className="mt-2 text-sm text-negative" role="alert">Google sign-in availability could not be checked.</p> : null}
      ...
      <div className="mt-5 overflow-x-auto border-y border-default" data-testid={PEOPLE_ACCESS_TEST_IDS.peopleTable}><table className="w-full min-w-180 text-left text-sm"><thead className="text-xs text-secondary"><tr><th className="px-3 py-2">Person</th><th className="px-3 py-2">Email</th><th className="px-3 py-2">Role</th><th className="px-3 py-2">Status</th><th className="px-3 py-2">Last sign-in</th><th className="px-3 py-2" aria-label="Actions" /></tr></thead><tbody className="divide-y divide-default">{(usersQuery.data?.users ?? []).map(user => <UserRow key={user.id} user={user} pending={pending} onUpdate={(row, body) => { void changeUser(row, body) }} onRevoke={() => { void run(async () => { await revokeUserAccess(user.id); await queryClient.invalidateQueries({ queryKey: USERS_KEY }) }) }} onHistory={() => setSelectedHistoryId(user.id)} />)}</tbody></table></div>
