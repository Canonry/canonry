...
Advanced tracked query scope and class filters must both honor URL selection. Simple tracked rows have no assignment classes, so retain their complete list and omit the class control even when the shared report URL carries a class filter. Retired scope keys require explicit recovery before scoped edit actions are available. Reuse the group-first visibility scope picker with explicit workspace parent metadata, and expose assignment membership behind an accessible disclosure. Within a Group, expand Subgroups before a collapsed All properties shortcut; leaf Properties remain expanded and collapsible. Reopening restores the selected Group or an explicit parent of the selected Property. Search results remain visible across matching levels. Research is an editable query builder for every authorized role. Groups organize the portfolio browser and never enter prompts or fan out runs. Select one explicit configured market or property per batch; expand a selected template once into the editor through the shared template helper, then send exactly the final entered question. Keep freeform research runnable without a plan and when hierarchy reads fail; fail closed only for explicit market/property selections. Load permitted query-tracking and template reads for Analysts without settings. Offer only project-configured templates, starting with Custom query. Expand only declared variables through the shared research helper so the editor and saved provenance agree. Returning to Whole site clears template selection and revision state while retaining the final text. Deduplicate normalized query identities while preserving the first exact text. Save template provenance and destination separately from final question text, show final-text classification, and carry the saved destination into property-assignment review. Provider location remains independent. Research labels describe project brand-name/domain checks, not verified property identity; keep saved answer engine/model/location context visible and render answer Markdown with safe external links and no remote images.
...
Research run controls consume the history response's `access.canRun` and `access.dailyRunLimit`. A narrow API-key browser gets the same direct research workspace and safe provider reads, without general write controls. Do not gate research on the coarse `canWrite` flag or duplicate its server capability rule.
