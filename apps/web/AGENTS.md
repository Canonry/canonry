...
Tracked query scope and class filters must both honor URL selection. Reuse the group-first visibility scope picker with explicit workspace parent metadata, and expose assignment membership behind an accessible disclosure. Research labels describe project brand-name/domain checks, not verified property identity; keep saved answer engine/model/location context visible and render answer Markdown with safe external links and no remote images.
